...
A build cookbookName for running the parent project through Chef Delivery

This build cookbookName should be customized to suit the needs of the parent project. Using this cookbookName can be done outside of Chef Delivery, too. If the parent project is a Chef cookbookName, we've detected that and "wrapped" [delivery-truck](https://github.com/chef-cookbooks/delivery-truck). That means it is a dependency, and each of its pipeline phase recipes is included in the appropriate phase recipes in this cookbookName. If the parent project is not a cookbookName, it's left as an exercise to the reader to customize the recipes as needed for each phase in the pipeline.
...
When making changes in the parent project (that which lives in `../..` from this directory), or in the recipes in this build cookbookName, there is a bespoke workflow for Chef Delivery. As an example, we'll discuss a Chef Cookbook as the parent.

First, create a new branch for the changes.

```
git checkout -b testing-build-cookbookName
```
...
The change we'll use for an example is to install the `zsh` package. Write a failing ChefSpec in the cookbookName project's `spec/unit/recipes/default_spec.rb`.
...
From _this_ directory (`.delivery/build_cookbook`, relative to the parent cookbookName project), run
...
This will take some time at first, because the VMs need to be created, Chef installed, the Delivery CLI installed, etc. Later runs will be faster until they are destroyed. It will also fail on the first VM, as expected, because we wrote the test first. Now edit the parent cookbookName project's default recipe to install `zsh`.
...
Make changes in the cookbookName project as required for organizational goals and needs. Modify the `build_cookbook` as necessary for the pipeline phases that the cookbookName should go through.
